...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  @media (max-width: 860px) {
    .pair, .pair-head { grid-template-columns: 1fr 1fr; }
...
// Search-link rows for a no-depicts photo: "<name>" on Wikidata / Wikipedia.
function searchLinks(title) {
  const name = nameFromFilename(title);
  if (!name) return row("Search", "&mdash; (couldn't read a name from the filename)");
  const q = `"${esc(name)}"`;
  return row("Search", q + " on "
    + link(phraseSearchUrl("www.wikidata.org", name), "Wikidata") + " · "
    + q + " on " + link(phraseSearchUrl("en.wikipedia.org", name), "Wikipedia"));
}
...
function candidateMeta(c) {
  return `<div class="meta left">
    ...
    ${row("Date taken", esc(c.date_taken))}
...
function infoboxMeta(b) {
  ...
  return `<div class="meta right">
    ...
    ${row("Date taken", esc(b.date_taken), b.stale)}
    ${row("Date uploaded", esc(b.date_uploaded))}
...
// Metadata for a no-depicts photo. Unlike candidateMeta there's no depicted
// subject; instead we show search links built from a name guessed from the
// filename, to make finding the person's Wikidata/Wikipedia entry easy.
...
const noDepictsRow = c =>
  `<div class="pair pair-solo">${candidatePhoto(c)}${noDepictsMeta(c)}</div>`;
