...
// Search URL for a person's name on a wiki, restricted to article namespace.
//
// Uses a PLAIN (unquoted) search, which finds the person best: measured against
// the alternatives on en.wikipedia, plain search ranks the actual article first
// for names that have one (James Dow, Sharon Horgan, Kate Griggs), while an
// exact-phrase ("Name") search buries or misses it -- 0 hits for A. Y. Chao, and
// unrelated top hits for Hanna Flint -- and intitle:"Name" returns nothing at all
// for people whose article title isn't an exact match. Quoting also made
// MediaWiki try the phrase as a page title, adding a "does not exist" notice.
//
// Note: this is a query parameter, so spaces are "+" encoded by URLSearchParams.
// Do not hand-build this as a /wiki/Special:Search/<name> path -- in a path
// segment "+" is a literal plus, which corrupted names into "A.+Y.+Chao".
function personSearchUrl(host, name) {
  const p = new URLSearchParams({
    title: "Special:Search", search: name, fulltext: "1", ns0: "1",
  });
...
function searchMeta(title) {
  ...
  return `<div class="meta right">
    ${row("Search", "<strong>" + esc(name) + "</strong> on "
      + link(personSearchUrl("www.wikidata.org", name), "Wikidata") + " · "
      + link(personSearchUrl("en.wikipedia.org", name), "Wikipedia"))}
  </div>`;
}
...
// `lang` labels which wiki the result came from when several were checked. Each
// result is its own .prev-result block so consecutive wikis are visibly split.
function previousPhotoHtml(p, lang) {
  const head = `<div class="prev-lang">${esc(lang)}</div>`;
  if (!p.found) {
    return { photo: "",
             meta: `<div class="prev-result">${head}${row("Previous photo", esc(p.reason))}</div>` };
  }
  return {
    photo: `<img src="${esc(p.thumb)}" alt="" loading="lazy">`,
    meta: `<div class="prev-result">` + head
...
document.addEventListener("click", async e => {
  ...
  // Clear any earlier lookup for this row: re-clicking replaces its results
